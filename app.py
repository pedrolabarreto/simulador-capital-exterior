...
years = st.sidebar.number_input("Prazo (anos)", 1, 50, 10)
...
def simulate_etf():
    ...
    for _ in range(years):
        ...
        tax_us += dividend * 0.30           # retenção 30% EUA
        value = (value + dividend * 0.70) * (1 + usd_growth_etf)
    gain = value - usd_initial
    tax_br = gain * 0.15                    # IR Brasil sobre ganho de capital
    tax_br_div = max(dividend * 0.15 - dividend * 0.15, 0)  # crédito máximo 15% (zero)
    return value, tax_us, tax_br + tax_br_div
...
def simulate_bond():
    ...
    for _ in range(years):
        ...
        tax_br += coupon * 0.15             # IR Brasil anual 15%
        coupons += coupon * 0.85            # reinvestimento líquido (simplificado)
        coupons *= (1 + 0.02)               # assume reinvestimento a 2% USD
...
def simulate_mutual():
    ...
    gain = value - usd_initial
    tax_br = gain * 0.15                    # IR Brasil somente no resgate
...
data = {
    "Cenário": ["ETF c/ Dividendos", "Bond c/ Cupom", "Mutual Fund Acumulativo"],
